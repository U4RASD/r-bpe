...
class BPETokenizerTrainer:
    def __init__(
        self, vocab_size, model_id, special_tokens_dict, dataset_dir=None, dataset=None
    ):
        if dataset_dir:
    ...
    def _prepare_dataset_from_dir(self, dataset_dir):
        ...
        if os.path.isfile(os.path.join(dataset_dir, "dataset_info.json")):
            ...
            datasets.extend(
                dataset.values() if isinstance(dataset, dict) else [dataset]
            )
        else:
    ...
    def _initialize_tokenizer(self):
        ...
        if (
            ...
            and self.special_tokens_dict.get("unk_token") is not None
            and self.special_tokens_dict.get("unk_token") != "None"
        ):
        ...
        tokenizer.pre_tokenizer = pre_tokenizers.Sequence(
            [
                pre_tokenizers.Split(pattern=gpt4_pattern, behavior="isolated"),
    ...
    def _wrap_tokenizer(self, tokenizer):
        try:
            tokenizer_class = (
                AutoTokenizer.from_pretrained(self.model_id).__class__
                if self.model_id
                else None
            )
            tokenizer_kwargs = {
                key: self.special_tokens_dict[key]
                for key in [
                    "unk_token",
                    "bos_token",
                    "eos_token",
