...
def main():
    ...
    with open(unified_aliases_file, "w", encoding="utf-8") as f:
        json.dump(
            dict(sorted(unicode_data_generator.unified_aliases.items())),
            f,
            indent=2,
            ensure_ascii=False,
        )
    print(
        f"    {len(unicode_data_generator.unified_aliases)} unified aliases mapped to aliases"
    )

    print(f"  - {name_to_alias_file}")
    with open(name_to_alias_file, "w", encoding="utf-8") as f:
        json.dump(
            dict(sorted(unicode_data_generator.script_name_to_alias.items())),
            f,
            indent=2,
            ensure_ascii=False,
        )
    print(
        f"    {len(unicode_data_generator.script_name_to_alias)} script names mapped to aliases"
    )

    print(f"  - {alias_to_name_file}")
    with open(alias_to_name_file, "w", encoding="utf-8") as f:
        json.dump(
            dict(sorted(unicode_data_generator.alias_to_script_name.items())),
            f,
            indent=2,
            ensure_ascii=False,
        )
    print(
        f"    {len(unicode_data_generator.alias_to_script_name)} aliases mapped to script names"
    )
