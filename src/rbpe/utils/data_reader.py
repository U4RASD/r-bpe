...
class DataReader:
    def __init__(self):
        ...
        self.block_to_scripts_file = (
            self.data_dir / "unicode_derived" / "block_to_scripts.json"
        )
        self.script_to_blocks_file = (
            self.data_dir / "unicode_derived" / "script_to_blocks.json"
        )
        self.script_name_to_alias_file = (
            self.data_dir / "unicode_derived" / "script_name_to_alias.json"
        )
        self.alias_to_script_name_file = (
            self.data_dir / "unicode_derived" / "alias_to_script_name.json"
        )
        ...
        with open(self.extensions_file, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if match:
                    ...
                    for cp in range(start, end + 1):
                        extensions_map[cp] = [
                            script.lower() for script in scripts_str.split()
                        ]

        return extensions_map

    def read_script_aliases(
        self,
