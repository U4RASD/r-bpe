...
class TokenClassifier:
    def __init__(
        ...
        self.unified_aliases, self.script_name_to_alias, self.alias_to_script_name = (
            self.data_reader.read_script_aliases()
        )
        self.blocks = self.data_reader.read_blocks()
        self.script_to_blocks = self.data_reader.read_json(
            self.data_reader.script_to_blocks_file
        )
        ...
        self.target_language_scripts = self._map_languages_to_aliases(
            target_language_scripts
        )
        self.preserved_languages_scripts = self._map_languages_to_aliases(
            preserved_languages_scripts
        )
        # Preserve both target and preserved languages
        self.preserved_languages_set = set(self.preserved_languages_scripts) | set(
            self.target_language_scripts
        )
    ...
    def _validate_params(
        ...
        if min_reusable_ids <= 0:
            raise ValueError(
                f"min_reusable_ids must be greater than 0: {min_reusable_ids}"
            )

        if not isinstance(target_language_scripts, list):
            raise ValueError(
                f"target_language_scripts must be a list: {target_language_scripts}"
            )
        if not isinstance(preserved_languages_scripts, list):
            raise ValueError(
                f"preserved_languages_scripts must be a list: {preserved_languages_scripts}"
            )

        if not self._validate_languages(target_language_scripts):
            raise ValueError(
                f"target_language_scripts must be valid script names or aliases: {target_language_scripts}"
            )
        if not self._validate_languages(preserved_languages_scripts):
            raise ValueError(
                f"preserved_languages_scripts must be valid script names or aliases: {preserved_languages_scripts}"
            )
    ...
    def _get_preserved_blocks_ranges(self) -> list:
        ...
        for block_name in preserved_block_names:
            if block_name in self.blocks:
                logger.debug(
                    f"Preserved block: {block_name}, ranges: {self.blocks[block_name]}"
                )
                preserved_ranges.append(self.blocks[block_name])

        self.preserved_blocks_ranges = preserved_ranges
        logger.info(
            f"Preserving {len(preserved_ranges)} blocks associated with scripts: {self.preserved_languages_scripts}"
        )
    ...
    def _classify_token(self, token: str, unicode_ranges: list) -> tuple:
        ...
        for char in token:
            script_language, char_range = self._find_unicode_range(
                ord(char), unicode_ranges
            )
        ...
        if len(found_languages) > 1:
            if all(
                lang in {"Katakana", "Hiragana", "CJK_merged"}
                for lang in found_languages
            ):
                found_languages = {
                    lang for lang in found_languages if "CJK" not in lang
                }
    ...
    def _classify_tokens_by_language(
        self, tokenizer: dict, unicode_ranges: list
    ) -> tuple:
        """
    ...
    def _load_tokenizer(self, model_id: str) -> dict:
        ...
        tokenizer_data = {"model": {"vocab": tokenizer.get_vocab(), "visible": {}}}
    ...
    def _classify_tokens(self) -> None:
        ...
        # Classify the tokens
        classified_tokens, classified_tokens_ids, classified_ranges = (
            self._classify_tokens_by_language(tokenizer_data, unicode_ranges)
        )
        ...
        total_classified_tokens = sum(
            len(tokens) for tokens in classified_tokens_ids.values()
        )
    ...
    def _analyze_tokenizer_languages(self) -> tuple:
        ...
        include_languages = [
            script.lower()
            for script in self.script_name_to_alias.keys()
            if script not in ["common", "inherited", "braille"]
        ]
        ...
        if any(script in self.preserved_languages_scripts for script in cjk_scripts):
            ...
            logger.info(
                f"CJK scripts detected. Updated preserved languages: {self.preserved_languages_scripts}"
            )
            cjk_case = True
        ...
        for language, data in language_data.items():
            ...
            if not any(
                included.lower() in language_parts for included in include_languages
            ):
            ...
            if ranges and self._ranges_overlap_with_preserved_blocks(ranges):
                ...
                if any(
                    preserved_language in language
                    for preserved_language in self.preserved_languages_scripts
                ) or (cjk_case and "cjk" in language):
                    logger.debug(
                        f"Excluding language '{language}' due to overlap with preserved blocks"
                    )
                    continue
        ...
        total_ids_available = sum(
            len(data["tokens"]) for data in language_data.values()
        )
        all_languages = [
            (lang, len(data["tokens"])) for lang, data in language_data.items()
        ]
    ...
    def write_sorted_languages_to_file(
        self, all_languages: list, output_file: str
    ) -> None:
    ...
    def _get_reusable_languages(self) -> None:
        ...
        """
        total_ids, selected_languages, total_ids_available, all_languages = (
            self._analyze_tokenizer_languages()
        )
        self.all_languages_data = all_languages

        logger.info(f"Total number of IDs available: {total_ids_available}")
        logger.info(
            f"Total number of reusable IDs (excluding languages containing {self.preserved_languages_scripts}): {total_ids}"
        )
        logger.info(
            f"Languages selected to reach or exceed {self.min_reusable_ids} reusable IDs:"
        )
        for lang in selected_languages:
            logger.info(f"  - {lang}")
        logger.info(
            f"Total number of languages selected for reuse: {len(selected_languages)}"
        )
        ...
        for language in self.reusable_languages:
            ...
            if language_data and "ranges" in language_data:
                ...
                reusable_languages_with_ranges_dict[language] = (
                    ranges  # Directly assign ranges list
                )
    ...
    def get_target_language_scripts_ranges(self) -> list:
        ...
        for script in self.target_language_scripts:
            if script in self.script_to_blocks:
                ...
                logger.debug(
                    f"Target language scripts blocks for script {script}: {blocks}"
                )
                for block in blocks:
