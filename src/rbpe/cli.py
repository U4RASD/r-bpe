...
@click.option(
    "--clean_data",
    help="Whether to clean the training data",
    default=True,
    type=click.BOOL,
)
@click.option(
    "--cleaned_data_dir", help="The directory containing the cleaned data", default=None
)
@click.option("--hf_token", help="The HuggingFace API token")
@click.option(
    "--min_reusable_count", help="The minimum number of reusable tokens", default=20000
)
@click.option(
    "--target_language_scripts",
    help="The target language scripts (comma-separated)",
    default="arabic",
)
@click.option(
    "--preserved_languages_scripts",
    help="The preserved languages scripts (comma-separated)",
    default="latin,greek",
)
@click.option(
    "--apply_rbpe_arabic_norm",
    help="Whether to apply the R-BPE Arabic normalization during encoding",
    default=True,
)
@click.option(
    "--special_tokens",
    help='Custom special tokens as JSON string, e.g. \'{"pad_token": "[PAD]", "unk_token": "[UNK]"}\'',
    default=None,
)
@click.option(
    "--additional_special_tokens",
    help="Additional special tokens (comma-separated)",
    default=None,
)
def create_tokenizer(
    ...
    hf_token,
    min_reusable_count,
    target_language_scripts=None,
    preserved_languages_scripts=None,
    apply_rbpe_arabic_norm=None,
    special_tokens=None,
    ...
    if target_language_scripts:
        target_language_scripts = [
            s.strip() for s in target_language_scripts.split(",")
        ]
    if preserved_languages_scripts:
        preserved_languages_scripts = [
            s.strip() for s in preserved_languages_scripts.split(",")
        ]
    ...
    if additional_special_tokens:
        additional_special_tokens = [
            s.strip() for s in additional_special_tokens.split(",")
        ]
    else:
        additional_special_tokens = []
        ...
        if not hf_token:
            ...
            if hf_token:
                ...
                raise click.BadParameter(
                    "hf_token is required. Please provide it via --hf_token or set HF_TOKEN environment variable."
                )
        if not model_id:
...
def classify_tokens(
    ...
    vocabulary_languages = sorted(
        token_classifier.all_languages_data, key=lambda x: x[1], reverse=False
    )
    result_dict = {}
