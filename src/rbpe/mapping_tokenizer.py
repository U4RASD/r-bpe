...
class MappingTokenizer:
    def __init__(
        ...
        self.old_tokenizer_last_special_token_id = self.old_tokenizer.all_special_ids[
            -1
        ]
        ...
        if (
            new_to_old_map_path is not None
            and old_to_new_map_path is not None
            and os.path.exists(new_to_old_map_path)
            and os.path.exists(old_to_new_map_path)
        ):
            self.new_to_old_map = self._load_token_map(new_to_old_map_path)
            self.old_to_new_map = self._load_token_map(old_to_new_map_path)
            self.replacement_character_map = self._load_token_map(
                replacement_character_map_path
            )
        else:
            self.reusable_token_ids, self.reusable_langs_unicode_ranges = (
                self.get_reusable_token_ids()
            )
        ...
        self.old_tokenizer_target_ids = [
            id
            for id in self.old_vocab.values()
            if self._is_target_input(self.old_tokenizer.decode([id]))
        ]
        self.new_tokenizer_target_ids = [
            id
            for _, id in self.new_vocab.items()
            if self._is_target_input(self.new_tokenizer.decode([id]))
        ]
        self.new_tokenizer_target_ids_mapped = [
            self.new_to_old_map[id] for id in self.new_tokenizer_target_ids
        ]
        ...
        if self.new_additional_tokens:
            ...
            escaped = [
                re.escape(t)
                for t in sorted(self.new_additional_tokens, key=len, reverse=True)
            ]
    ...
    def get_reusable_token_ids(self):
        ...
        for lang in self.reusable_languages:
            ...
            reusable_langs_unicode_ranges[lang] = self.token_id_language_map[lang][
                "ranges"
            ]
    ...
    def get_token_sets(self, new_tokenizer, original_tokenizer):
        ...
        new_tokenizer_tokens = set(
            [token for token in new_tokenizer.get_vocab().keys()]
        )
        original_tokenizer_tokens = set(
            [token for token in original_tokenizer.get_vocab().keys()]
        )
        ...
        logger.debug(
            f"New tokenizer vocabulary size: {len(new_tokenizer_tokens)} tokens"
        )
    ...
    def _create_token_map(self):
        ...
        new_tokens, common_tokens = self.get_token_sets(
            self.new_tokenizer, self.old_tokenizer
        )
        new_tokens = [token for token in new_tokens]

        self.common_token_ids = [
            self.old_vocab[token]
            for token in tqdm(common_tokens, desc="Getting common token IDs")
        ]
        ...
        mapping = {
            self.new_vocab[token]: old_id
            for token, old_id in tqdm(
                zip(new_tokens, self.reusable_token_ids), desc="Mapping new tokens"
            )
        }

        mapping.update(
            {
                self.new_vocab[token]: self.old_vocab[token]
                for token in tqdm(common_tokens, desc="Mapping common tokens")
            }
        )
    ...
    def _is_target_input(self, text):
        ...
        for char in text:
            ...
            if any(
                ...
                for start, end in self.target_language_scripts_ranges
            ):
                return True
        return False
    ...
    def encode(self, text, **kwargs):
        ...
        for segment_text, is_always_new in initial_segments:
            ...
            normalized_segment_text = (
                self.unicode_normalizer.normalize(segment_text)
                if self.apply_normalization
                else segment_text
            )
            for segment_text, is_target in self._segment_input_text(
                normalized_segment_text
            ):
                if is_target:
                    ...
                    if self.debug_mode:
                        ...
                        for id in new_ids:
                            logger.debug(
                                f"New ID: {id}, Old ID: {self.new_to_old_map[id]}"
                            )
                else:
    ...
    def _find_optimal_window(
        self, ids, start_idx, current_segment=None, current_is_mapped=None, **kwargs
    ):
        ...
        max_window_size = min(
            4, len(ids) - start_idx
        )  # At most 4 tokens (current + 3 more)
        best_window_size = 1
        best_decoded = None
        best_is_mapped = None
        ...
        for window_size in range(1, max_window_size + 1):
            ...
            window_has_unmapped = any(
                tid not in self.old_to_new_map for tid in test_window
            )

            # Try decoding with old tokenizer if any token is unmapped
            if window_has_unmapped or (
                current_is_mapped is not None and not current_is_mapped
            ):
    ...
    def decode(self, ids, **kwargs):
        ...
        while i < len(ids):
            token_id = ids[i]
            is_byte = token_id > old_last_special and token_id <= (
                256 + old_last_special
            )
            ...
            if self.debug_mode:
                logger.debug(
                    f"ID: {token_id}, is_replacement: {is_replacement}, is_common_token: {is_common_token}, is_mapped: {is_mapped}, is_byte: {is_byte}"
                )
            ...
            if current_is_mapped is None:
                ...
                if is_replacement:
                    best_window_size, best_decoded, best_is_mapped = (
                        self._find_optimal_window(
                            ids, i, current_segment, current_is_mapped, **kwargs
                        )
                    )
            ...
            if is_replacement:
                ...
                if best_decoded is not None:
                    if best_is_mapped == current_is_mapped:
                        ...
                        segments.append(
                            (current_segment, current_is_mapped)
        ...
        # Decode each segment with the appropriate tokenizer
        decoded_segments = [
            self._decode_segment(segment, is_mapped, **kwargs)
            for segment, is_mapped in segments
        ]
    ...
    def __setstate__(self, state):
        ...
        self.unicode_normalizer = (
            UnicodeNormalizer() if self.apply_normalization else None
        )
    ...
    def to_json(self):
        ...
        return (
            json.dumps(self.__getstate__(), ensure_ascii=False).encode("utf8").decode()
        )
    ...
    def from_json(cls, json_str):
        ...
        state["new_to_old_map"] = {
            int(k): v for k, v in state["new_to_old_map"].items()
        }
        state["old_to_new_map"] = {
            int(k): v for k, v in state["old_to_new_map"].items()
        }
        state["replacement_character_map"] = {
            int(k): v for k, v in state["replacement_character_map"].items()
        }
        state["common_token_ids_map"] = {
            int(k): v for k, v in state["common_token_ids_map"].items()
        }
